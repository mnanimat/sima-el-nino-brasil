...
import { EmergencyNewsFeed } from './components/EmergencyNewsFeed';
...
export default function App() {
  ...
  const handleConsultAdvisorWithNews = (title: string, summary: string) => {
    setChatInitialPrompt(`Qual é a análise técnica, impacto operacional e plano de ação recomendado para a seguinte ocorrência climática de emergência?\n\nEvento: "${title}"\nContexto: ${summary}`);
    setIsChatOpen(true);
  };
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        {activeTab === 'all' && (
          <div className="space-y-8">
            ...
            {/* Emergency News Feed Component (Google Search Live) */}
            <EmergencyNewsFeed
              initialRegion={selectedRegion || 'all'}
              onConsultAdvisorWithNews={handleConsultAdvisorWithNews}
            />
      ...
      <footer className="mt-auto border-t border-slate-800 bg-slate-950 py-8 text-xs text-slate-400">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4 pb-6 border-b border-slate-800/80">
            ...
            {/* Developer Credits Highlight & Contact */}
            <div className="p-3 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-center gap-3 text-xs text-slate-300">
              ...
              <div className="flex items-center gap-1.5 w-full sm:w-auto justify-end">
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/60 text-emerald-300 hover:text-emerald-200 text-[11px] font-bold border border-emerald-800/60 transition-colors flex items-center gap-1"
                >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px]">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-2">
              <button
                onClick={() => handleOpenLegalModal('contact')}
                className="hover:text-emerald-400 text-emerald-300 font-semibold transition-colors flex items-center gap-1"
              >
                <Mail className="w-3.5 h-3.5 text-emerald-400" />
                <span>Contato, Perguntas & Parcerias</span>
              </button>
